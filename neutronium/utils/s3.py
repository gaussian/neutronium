...
def upload_dir_to_s3(
    source_directory: str, s3_path: str, s3_bucket: str = None, archive: bool = True
):
    import subprocess
    import shutil
    import os
    ...
    if not archive:
        raise NotImplementedError(
            "Only collected/zipped directories supported for S3 upload"
        )
    ...
    # Upload to S3
    upload_to_s3(
        filename=temp_filename, s3_path=s3_path, s3_bucket=s3_bucket, compress=False
    )
...
def upload_to_s3(
    ...
    if not filename and not content:
        raise ValueError(
            f"Need filename or content defined to upload to S3, requested path is {s3_path}"
        )
        ...
        if compress:
            with open(filename, "rb") as original_file_obj:
                ...
                response = s3.put_object(
                    Body=compressed_file_obj.getvalue(), **put_params
                )
...
def clear_s3_dir_safe(
    s3_bucket: str, s3_dir: str, aws_access_key_id=None, aws_secret_access_key=None
):
    delete_limit = 10
    ...
    for _ in bucket.objects.filter(Prefix=f"{s3_dir}/"):
        ...
        if count >= delete_limit:
            raise ValueError(
                f"Too many objects to delete in {s3_bucket}/{s3_dir}, will not execute"
            )
